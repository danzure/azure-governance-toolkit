...
To maintain a unified Fluent 2 design language, the following legacy UI patterns are **STRICTLY FORBIDDEN**:
- **Native HTML `<select>` Elements**: Do not use native browser `<select>` dropdowns anywhere in the application, as their rendering (blue highlight, system fonts) violates the Fluent 2 design system. Always use or build a custom React dropdown component (such as `SearchableSelect.jsx` or an absolute-positioned `div` menu) featuring proper `bg-fluent-bg-card` backgrounds, `hover:bg-fluent-bg-hover` interactive states, `shadow-flyout`, and standardized icons (`ChevronDown`).
- **Hardcoded & Generic Colors**: Do not use raw hex colors (e.g., `bg-[#292929]`, `text-[#D4D4D4]`) or generic Tailwind colors (e.g., `bg-white`, `bg-blue-600`, `text-gray-300`). Always use the corresponding `fluent-*` semantic tokens defined below.
- **Arbitrary Hover States**: Avoid ad-hoc hover background definitions like `hover:bg-black/5` or `dark:hover:bg-white/5`. Use the standardized `hover:bg-fluent-bg-hover` or `hover:bg-fluent-bg-subtle`.
- **Legacy Shadows**: Do not use generic Tailwind shadows like `shadow-md` or `shadow-lg`. Use `shadow-soft`, `shadow-depth`, or `shadow-flyout`.
- **Inconsistent Button/Input Padding**: Do not use ad-hoc padding/sizing like `px-4 py-2` or `px-3 py-2.5`. Use the standardized `px-3 h-[32px]` format.
...
- **Helper/Description Text**: `text-[12px] text-fluent-fg-secondary mt-1 block`
- **Text Inputs (`<input type="text">`)**: `flex-1 min-w-0 w-full px-3 h-[32px] border rounded outline-none text-[14px] transition-all duration-200 focus:border-fluent-brand-bg focus:ring-2 focus:ring-fluent-brand-bg/20 bg-fluent-bg-card text-fluent-fg-primary border-fluent-stroke-strong placeholder:text-fluent-fg-tertiary`
- **Dropdowns & Select Menus**: **Never** use native HTML `<select>` tags. Implement custom dropdowns using absolute positioned panels (`absolute top-[100%] z-50 bg-fluent-bg-card border border-fluent-stroke-subtle shadow-flyout rounded mt-1`). Dropdown trigger buttons must use `flex items-center justify-between` with a trailing `ChevronDown` that rotates on open (`rotate-180`). Dropdown items should use `px-3 py-2 text-[12px] text-fluent-fg-secondary hover:bg-fluent-bg-hover hover:text-fluent-fg-primary cursor-pointer transition-colors`.
- **Integrated Input Dropdowns (Action Selects)**: When appending a custom dropdown menu (e.g., an "Examples" button) directly to a text input field, they must be seamlessly integrated. Wrap both the input and the dropdown trigger in a shared container: `flex items-center w-full h-[32px] border rounded transition-all duration-200 focus-within:border-fluent-brand-bg focus-within:ring-2 focus-within:ring-fluent-brand-bg/20 bg-fluent-bg-canvas border-fluent-stroke-strong overflow-visible relative`. The inner `<input>` must use `flex-1 min-w-0 px-3 h-full outline-none text-[13px] bg-transparent border-none`. The inner dropdown trigger wrapper should use `h-full border-l border-fluent-stroke-subtle bg-fluent-bg-subtle flex items-center shrink-0 relative`.
...
| `Check` | Success state, especially after a successful copy action. |

...
export default function TopologyCodeGenerator({ topology }) {
    ...
    return (
        <div className="bg-fluent-bg-canvas border border-fluent-stroke-subtle rounded-xl shadow-soft flex flex-col overflow-hidden">
            <div 
                className="px-5 py-4 flex flex-col lg:flex-row lg:items-center justify-between cursor-pointer hover:bg-fluent-bg-subtle transition-colors gap-4 lg:gap-0"
                onClick={() => setIsExpanded(!isExpanded)}
            >
                <div className="flex items-center gap-3 text-fluent-fg-primary font-semibold select-none">
                    <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-fluent-brand-bg/10 text-fluent-brand-fg">
                        <Code className="w-4 h-4" />
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[15px]">Infrastructure Code</span>
                        {isExpanded && <span className="text-[12px] font-normal text-fluent-fg-secondary">Auto-generated {format === 'terraform' ? 'Terraform' : format === 'bicep' ? 'Bicep' : 'ARM'} template for your topology</span>}
                    </div>
                </div>
                
                <div className="flex items-center gap-2">
                    {isExpanded && (
                        <div className="flex flex-wrap items-center gap-3 mr-4" onClick={(e) => e.stopPropagation()}>
                        <div className="flex shrink-0 bg-[#edebe9] dark:bg-[#323130] rounded-sm p-0.5">
                            <button
                                onClick={() => setFormat('bicep')}
                                className={`text-[12px] px-3 py-1 font-medium rounded-sm transition-all ${format === 'bicep' ? 'bg-white dark:bg-[#484644] text-[#0078d4] dark:text-[#60cdff] shadow-sm' : 'text-[#605e5c] dark:text-[#c8c6c4] hover:text-[#323130] dark:hover:text-[#e1dfdd]'}`}
                            >
                                Bicep
                            </button>
                            <button
                                onClick={() => setFormat('arm')}
                                className={`text-[12px] px-3 py-1 font-medium rounded-sm transition-all ${format === 'arm' ? 'bg-white dark:bg-[#484644] text-[#0078d4] dark:text-[#60cdff] shadow-sm' : 'text-[#605e5c] dark:text-[#c8c6c4] hover:text-[#323130] dark:hover:text-[#e1dfdd]'}`}
                            >
                                ARM
                            </button>
                            <button
                                onClick={() => setFormat('terraform')}
                                className={`text-[12px] px-3 py-1 font-medium rounded-sm transition-all ${format === 'terraform' ? 'bg-white dark:bg-[#484644] text-[#0078d4] dark:text-[#60cdff] shadow-sm' : 'text-[#605e5c] dark:text-[#c8c6c4] hover:text-[#323130] dark:hover:text-[#e1dfdd]'}`}
                            >
                                Terraform
                            </button>
                        </div>
                        ...
                        >
                            ...
                            {format === 'terraform' ? 'Terraform Registry' : format === 'bicep' ? 'Bicep Template' : 'ARM Template'}
                    ...
                    <div className="text-fluent-fg-tertiary ml-2">
